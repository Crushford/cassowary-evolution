...
function layoutForCount(cardCount: number) {
  ...
  if (cardCount === 40) return { kind: "columnsOfRows" as const, rows: 8, cols: 5, columns: 1 };
  if (cardCount === 80) return { kind: "columnsOfRows" as const, rows: 16, cols: 5, columns: 1 };
  // fallback grid
...
// ---------- evolution nodes ----------
function starterEvolutionNodes(): EvolutionNode[] {
  return [
    {
      id: "eggs-1",
