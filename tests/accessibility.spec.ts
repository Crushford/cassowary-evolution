...
async function navigateWithKeyboard(page: Page, direction: 'Tab' | 'Shift+Tab' | 'ArrowUp' | 'ArrowDown' | 'ArrowLeft' | 'ArrowRight', count: number = 1) {
...
async function selectTileByKeyboard(page: Page, targetRow: number, targetCol: number) {
  ...
  const targetTile = page.locator(`button[aria-label*="row ${targetRow}, column ${targetCol}"]`);
...
async function playRound(page: Page, tileSelections: Array<{row: number, col: number}>) {
...
test.describe('Accessibility Tests', () => {
  ...
  test('Game is keyboard navigable', async ({ page }) => {
    ...
    const focusableElements = await page.locator('button, [tabindex="0"], input, select, textarea').count();
  ...
  test('Game board has proper ARIA labels and roles', async ({ page }) => {
    ...
    await expect(gameGrid).toHaveAttribute('aria-label', 'Game board with 9x9 grid of tiles');
  ...
  test('Tile selection works with keyboard', async ({ page }) => {
    ...
    const selectableTile = page.locator('button[aria-label*="Unknown territory"]').first();
    ...
    // Check that the tile is now selected
    await expect(selectableTile).toHaveAttribute('aria-pressed', 'true');
    
    // Check that the aria-label updated to reflect selection
    const updatedAriaLabel = await selectableTile.getAttribute('aria-label');
    expect(updatedAriaLabel).toContain('Selected');
  });
  ...
  test('Screen reader can navigate game state', async ({ page }) => {
    ...
    const gameStatusBar = page.locator('[role="complementary"]');
    ...
    // Check that chip count is accessible
    const chipInfo = page.locator('text=/\\d+ chips/');
    await expect(chipInfo).toBeVisible();
    
    // Check that partner count is accessible
    const partnerInfo = page.locator('text=/\\d+ partners/');
