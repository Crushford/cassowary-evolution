...
class KeyboardGameplayHelper {
  ...
  async waitForGameReady() {
    ...
    await this.page.waitForSelector('button[aria-label*="Tile at row"]', { timeout: 5000 });
  }
  ...
  async selectTileByRowCol(row: number, col: number) {
    ...
    if (isDisabled) {
      throw new Error(`Tile at row ${row}, column ${col} is disabled (likely Queen's nest)`);
    }
  ...
  async layEggs() {
    ...
    let layEggsButton = this.page.locator('button[aria-label*="Lay eggs"]:not([disabled])');
  ...
  async getGameState() {
    return await this.page.evaluate(() => {
      ...
      const partnersElement = allSpans.find(span => span.textContent?.includes('partners'));
  ...
  async playCompleteRound(tileSelections: Array<{row: number, col: number}>) {
    ...
    for (const selection of tileSelections) {
      ...
      if (!selected) {
        throw new Error(`Failed to select tile at row ${selection.row}, column ${selection.col}`);
      }
    }
    ...
    const resultsVisible = await this.page.locator('h3:has-text("Round Results")').isVisible();
    if (!resultsVisible) {
...
test.describe('Keyboard-Only Gameplay - First 3 Levels', () => {
  ...
  test('Complete keyboard-only gameplay through first 3 levels', async ({ page }) => {
    ...
    for (let round = 1; round <= 3; round++) {
      ...
      console.log(`Initial state - Era: ${initialState.era}, Chips: ${initialState.chips}, Partners: ${initialState.partners}`);
      ...
      console.log(`Final state - Era: ${finalState.era}, Chips: ${finalState.chips}, Partners: ${finalState.partners}`);
  ...
  test('Keyboard navigation through all game elements', async ({ page }) => {
    ...
    for (let i = 0; i < 20; i++) { // Limit to prevent infinite loops
      ...
      if (elementInfo.tagName && elementInfo.tagName !== 'BODY') {
        ...
        console.log(`Element ${i}: ${elementInfo.tagName} - ${elementInfo.ariaLabel || elementInfo.textContent}`);
      }
      ...
      if (i > 0) {
        ...
        if (currentInfo.tagName === firstInfo.tagName && 
    ...
    const gameElements = focusableElements.filter(el => 
  ...
  test('Error handling and edge cases', async ({ page }) => {
    ...
    if (isDisabled) {
      console.log('✅ Lay eggs button correctly disabled when insufficient tiles selected');
    } else {
      ...
      const resultsVisible = await page.locator('h3:has-text("Round Results")').isVisible();
      expect(resultsVisible).toBe(false);
      console.log('✅ Lay eggs button correctly prevented action with insufficient tiles');
    }
    ...
    console.log(`Focus after modal interaction: ${focusedInfo.tagName} - ${focusedInfo.ariaLabel}`);
