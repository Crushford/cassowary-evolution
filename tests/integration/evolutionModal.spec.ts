...
describe('Evolution Modal Integration', () => {
  ...
  describe('Modal visibility based on EP', () => {
    ...
    test('should show all available tiers at EP 50', () => {
      ...
      const tiers = [...new Set(visibleNodes.map(node => node.tier))].sort();
  ...
  describe('Node purchase validation', () => {
    const createGameState = (ep: number, purchasedNodes: string[] = []): Partial<GameState> => ({
      progress: {
        evolutionPoints: ep,
        population: 100,
        currentLevel: 1,
    ...
    test('should validate EP cost requirements', () => {
      ...
      const canAffordDigestion1 = gameState.progress!.evolutionPoints >= digestion1Node.cost;
    ...
    test('should validate prerequisite requirements', () => {
      ...
      const canBuyEggs2 = eggs2Node.prereq?.every(prereqId => 
        gameState.progress!.purchasedNodes.includes(prereqId)
      ) ?? true;
      
      const canBuyDigestion2 = digestion2Node.prereq?.every(prereqId => 
        gameState.progress!.purchasedNodes.includes(prereqId)
      ) ?? true;
  ...
  describe('Node cost and tier relationship', () => {
    ...
    test('should have reasonable cost progression within tiers', () => {
      const nodesByTier = config.evolution.nodes.reduce((acc, node) => {
  ...
  describe('Complex purchase scenarios', () => {
    ...
    test('should handle multiple prerequisites', () => {
      ...
      const canBuyEggs2 = gameState.progress!.evolutionPoints >= eggs2Node.cost &&
  ...
  function createGameState(ep: number, purchasedNodes: string[] = []): Partial<GameState> {
    return {
      progress: {
        evolutionPoints: ep,
        population: 100,
