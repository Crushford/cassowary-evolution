...
describe('Board Transition Selection', () => {
  ...
  describe('No downsize behavior', () => {
    test('should not downsize when population drops within a cycle', () => {
      ...
      // Drop population but stay in same cycle
      const droppedLevel = getCurrentLevel(50, cycle);
      expect(droppedLevel!.cardCount).toBe(20); // Should still be valley, not glade
    });
  ...
  describe('Layout consistency', () => {
    test('should have consistent layout for each card count', () => {
      ...
      Object.entries(cardCountLayouts).forEach(([cardCount, expectedLayout]) => {
        const level = getCurrentLevel(parseInt(cardCount) === 5 ? 1 : parseInt(cardCount) === 10 ? 10 : parseInt(cardCount) === 15 ? 50 : parseInt(cardCount) === 20 ? 100 : parseInt(cardCount) === 40 ? 200 : 400, 0);
