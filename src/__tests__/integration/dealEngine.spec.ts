import { makeDeal } from '../../game/deal';
import { getOddsTable } from '../../game/config';
...
describe('Deal Engine + Odds Integration', () => {
  describe('Deal composition with fruitConstantL1 odds', () => {
    ...
    test('should produce different results with different seeds', () => {
      ...
      makeDeal('seed-1', 1, recipe);
      makeDeal('seed-2', 1, recipe);
  ...
  describe('Deal randomness distribution', () => {
    ...
    test('should maintain composition across different card counts', () => {
      ...
      recipes.forEach(({ tileCount, composition }) => {
        ...
        expect(deal.filter((outcome) => outcome === 'fruit')).toHaveLength(
          composition.fruit,
        );
        expect(deal.filter((outcome) => outcome === 'barren')).toHaveLength(
          composition.barren,
        );
        expect(deal.filter((outcome) => outcome === 'predator')).toHaveLength(
